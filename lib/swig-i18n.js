...
exports.init = function (translation_thing, config, next) {
  ...
  swig.setTag('i18n', function(str, line, parser, types) {
    ...
    parser.on(types.COLON, function (token) { 
      if ( handlingSubs ) {
        this.out.push('(' + isDefined('_ctx.', accumulatedJS) + ' ? ' + '_ctx.' + accumulatedJS + ' : ' + isDefined('',accumulatedJS) + ' ? ' + accumulatedJS + ' : "' + accumulatedJS + '")');
      }
      accumulatedJS = '';
      return false;
    });
    ...
    parser.on(types.COMMA, function (token) { 
      if ( handlingSubs && endIsSafe) {
        this.out.push('(' + isDefined('_ctx.', accumulatedJS) + ' ? ' + '_ctx.' + accumulatedJS + ' : ' + isDefined('',accumulatedJS) + ' ? ' + accumulatedJS + ' : "")');
      }
      endIsSafe     = true;
      accumulatedJS = '';
      return false;
