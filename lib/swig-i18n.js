    ...
    var generate_output_code = function (token) {
      ...
      if ( currentState == STATES.BEGIN ) { // Variable or string
        ...
        if (currentState === STATES.KEY) { /* key */
          ...
          if ( accumulatedValue.match(/^[\'\"].*[\'\"]$/) ) {  // quoted string
            match = ',("' + accumulatedValue.replace(/[\'\"]/g,'') + '"));';
          } else if ( accumulatedValue.match(/^\d+$/) ) { //unquoted number
            match = ',(' + accumulatedValue + '));';
          } else { // last ditch effort to find it as a variable in ctx var
            match = ',(' + isDefined('_ctx.', accumulatedValue) + ' ? ' + '_ctx.' + accumulatedValue + ' : accumulatedValue));';
          }
          this.out.push(match);
        }
      }
