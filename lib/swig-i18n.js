...
exports.init = function (translation_thing, config, next) {
  ...
  function translate(tag, language, default_translation) {
    if ( config.imperial_language && 
            (config.imperial_language === language || config.active_languages && !_.contains(config.active_languages, language)) && 
                    default_translation) {
  ...
  function(compiler, args, content, parents, options, blockName) {
    ...
    var content = compiler(content, parents, options, blockName);
    if (config.analyze) {
    ...
    while(args.length) {
      ...
      output += 'var replace_with = ';
      if (typeof(config.autoescape) === 'string') {
          output += '_filters["escape"](' + replace_with + ',"' + config.autoescape + '");\n';
      } else {
          output += replace_with + ';\n';
      }
